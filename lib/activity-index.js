...
export const DEFAULT_LEDGER = 'out/activity-index-credits.json';
export const DEFAULT_MONTHLY_CAP = 40000;
...
// Does the record's NAME plausibly belong to the site? Used only to gate the
// homepage-declared fallback: web agencies put their own LinkedIn link in the
// footer of every site they build ("MarkUpgrade", 3 employees, accepted for a
// client's domain on 2026-09-16). A name token of 4+ letters must appear in the
// target domain or the site's own <title>. Generic words do not count.
const GENERIC = new Set(['group','groupe','inc','corp','corporation','company','limited','ltd','llc','plc','holdings','international','global','services','service','solutions','technology','technologies','systems','partners','associates','consulting','digital','agency','web','media','the','and']);
export function nameMatchesSite(name, { domain = '', title = '' } = {}) {
  const hay = (String(domain) + ' ' + String(title)).toLowerCase().replace(/[^a-z0-9]+/g, ' ');
  const hayCompact = hay.replace(/\s+/g, '');
  const tokens = String(name || '').toLowerCase().replace(/[^a-z0-9]+/g, ' ').split(' ').filter((t) => t.length >= 4 && !GENERIC.has(t));
  return tokens.some((t) => hay.includes(t) || hayCompact.includes(t));
}
export function extractTitle(html) {
  const m = /<title[^>]*>([^<]{1,300})<\/title>/i.exec(String(html || ''));
  const og = /property=["']og:site_name["'][^>]*content=["']([^"']{1,200})["']/i.exec(String(html || '')) || /content=["']([^"']{1,200})["'][^>]*property=["']og:site_name["']/i.exec(String(html || ''));
  return [m?.[1], og?.[1]].filter(Boolean).join(' ');
}
...
export function assessCandidate(record, { domain, finalHost = null, employeesIndexed = null, method = 'enrich_by_website', siteTitle = '' } = {}) {
  const checks = {};
  ...
  checks.name_matches_site = nameMatchesSite(record?.company_name, { domain: `${domain} ${finalHost || ''}`, title: siteTitle });
  ...
  else if (checks.declared_by_homepage && checks.has_headcount && checks.headcount_consistent && checks.name_matches_site) confidence = 'medium'; // alias domain; the company's own link, and the name fits the site
  else confidence = 'low';
...
export class ActivityIndex {
  constructor({ apiKey, baseUrl, maxCreditsPerRun = 60, cacheDir = DEFAULT_CACHE_DIR, ttlMs = CACHE_TTL_MS, fetchImpl = globalThis.fetch, timeoutMs = 20000, monthlyCap = DEFAULT_MONTHLY_CAP, ledgerPath = DEFAULT_LEDGER } = {}) {
    this.apiKey = apiKey || null;
    this.baseUrl = baseUrl ? String(baseUrl).replace(/\/+$/, '') : null;
    // The endpoint is public. A per-run ceiling bounds one visitor; the monthly
    // ledger bounds a month of them, and degrades to website-only when hit.
  ...
  // ---- monthly ledger (best-effort file; a missing or corrupt ledger counts as 0) ----
  _ledger() {
    const month = new Date().toISOString().slice(0, 7);
    try {
      if (this.ledgerPath && existsSync(this.ledgerPath)) {
        const l = JSON.parse(readFileSync(this.ledgerPath, 'utf8'));
        if (l.month === month && Number.isFinite(l.used)) return l;
      }
    } catch { /* fall through */ }
    return { month, used: 0 };
  }
  get monthlyUsed() { return this._ledger().used; }
  _charge(n) {
    if (!n || !this.ledgerPath) return;
    try {
      const l = this._ledger(); l.used += n; l.updated_at = new Date().toISOString();
      mkdirSync(dirname(this.ledgerPath), { recursive: true });
      writeFileSync(this.ledgerPath, JSON.stringify(l));
  ...
  async _call(method, path, { body = null, cost = 0, note = null } = {}) {
    ...
    if (cost > this.creditsRemaining || (cost > 0 && this.monthlyUsed + cost > this.monthlyCap)) {
      const entry = { method, path, note, status: 'ceiling', cost: 0, ms: 0, reason: cost > this.creditsRemaining ? 'per-run ceiling' : 'monthly cap' };
  ...
  async resolveCompany({ domain, finalHost = null, html = null, siteTitle = null, useCache = true } = {}) {
    siteTitle = siteTitle ?? extractTitle(html);
    ...
    const consider = async (method, fetchRecord) => {
      ...
      const a = assessCandidate(record, { domain: key, finalHost, employeesIndexed, method, siteTitle });
  ...
  async companyCounts(companyId, { days = 90, now = Date.now() } = {}) {
    ...
    // All seven in flight at once: they are free, independent, and a visitor is waiting.
    const [employees_indexed, employees_posted_in_window, employees_posting_12plus_per_year, decision_makers_posted_in_window, company_posts_all_time, company_posts_in_window, reshares_of_company_posts_in_window] = await Promise.all([
      q('employee_multi_source', [by], 'employees indexed'),
      q('employee_multi_source', [by, { range: { posting_recency: { gte: since } } }], 'employees posted'),
      q('employee_multi_source', [by, { range: { post_frequency_yearly: { gte: 12 } } }], 'employees 12+/yr'),
      q('employee_multi_source', [by, { term: { is_decision_maker: 1 } }, { range: { posting_recency: { gte: since } } }], 'decision makers posted'),
      q('company_post', [co], 'company posts all'),
      q('company_post', [co, { range: { date_published: { gte: since } } }], 'company posts window'),
      // NESTED is required. The flat term form answers 0 for every company (probe 2026-09-16).
      q('employee_post', [{ nested: { path: 'reshared_post', query: { term: { 'reshared_post.company_id': companyId } } } }, { range: { date_published: { gte: since } } }], 'reshares window'),
    ]);
    const out = { window_days: days, since, employees_indexed, employees_posted_in_window, employees_posting_12plus_per_year, decision_makers_posted_in_window, company_posts_all_time, company_posts_in_window, reshares_of_company_posts_in_window };
...
// The block that goes INTO the evidence file. Deliberately small: no raw company
// record (it carries emails, phone numbers and 100 posts), no candidates' names
// beyond what explains a rejection, nothing about any individual. The summary
// keeps the handful of record fields the report and lead score can use.
export function evidenceBlock(resolution, counts = null) {
  const r = resolution || { status: 'disabled' };
  const rec = r.record || null;
  const base = {
    source: 'licensed third-party index',
    status: r.status,
    fetched_at: new Date().toISOString(),
    credits_used: r.credits_used ?? 0,
  };
  if (r.status !== 'resolved') {
    return { ...base, reason: r.reason || null, candidates: (r.candidates || []).map((c) => ({ method: c.method, verdict: c.verdict, confidence: c.confidence ?? null, employees_indexed: c.employees_indexed ?? null })) };
  }
  return {
    ...base,
    confidence: r.confidence, coverage: r.coverage, method: r.method, company_id: r.company_id, company_name: r.company_name ?? null,
    record_employees_count: rec?.employees_count ?? null,
    record_summary: rec ? {
      website: rec.website ?? null, size_range: rec.size_range ?? null, industry: rec.industry ?? null, hq_country: rec.hq_country ?? null,
      followers: rec.followers_count_linkedin ?? rec.followers_count_professional_network ?? null,
      competitors: (rec.competitors || []).slice(0, 8).map((c) => c.company_name).filter(Boolean),
    } : null,
    candidates: (r.candidates || []).map((c) => ({ method: c.method, verdict: c.verdict, confidence: c.confidence ?? null, employees_indexed: c.employees_indexed ?? null })),
    counts,
  };
}

// One call for the pipeline: resolve, count, and return the evidence block plus
// the list of things that could not be observed. Never throws.
export async function observeActivity(index, { domain, finalHost = null, html = null } = {}) {
  const notObserved = [];
  if (!index || !index.enabled) {
    notObserved.push('Employee posting activity: no licensed activity index configured, so Employee & Culture is scored on website evidence alone.');
    return { block: evidenceBlock({ status: 'disabled', reason: 'not configured' }), not_observed: notObserved };
  }
  try {
    const r = await index.resolveCompany({ domain, finalHost, html });
    if (r.status !== 'resolved') {
      notObserved.push(`Employee posting activity: this domain could not be matched to a company in the licensed index (${r.reason || r.status}), so Employee & Culture is scored on website evidence alone.`);
      return { block: evidenceBlock(r), not_observed: notObserved };
    }
    const counts = r.counts || await index.companyCounts(r.company_id);
    if (counts.employees_indexed === null) notObserved.push('Employee posting activity: the licensed index did not answer the employee counts for this company.');
    return { block: evidenceBlock(r, counts), not_observed: notObserved };
  } catch (e) {
    notObserved.push(`Employee posting activity: the licensed index lookup failed (${String(e?.message || e).slice(0, 120)}), so Employee & Culture is scored on website evidence alone.`);
    return { block: evidenceBlock({ status: 'error', reason: String(e?.message || e).slice(0, 200) }), not_observed: notObserved };
  }
}

// Build a client from the merged env the way the rest of lib/ does it.
export function indexFromEnv(env = {}) {
  return new ActivityIndex({
    apiKey: process.env.ACTIVITY_INDEX_API_KEY || env.ACTIVITY_INDEX_API_KEY || null,
    baseUrl: process.env.ACTIVITY_INDEX_BASE_URL || env.ACTIVITY_INDEX_BASE_URL || null,
    maxCreditsPerRun: Number(process.env.ACTIVITY_INDEX_MAX_PER_RUN || env.ACTIVITY_INDEX_MAX_PER_RUN || 40),
    monthlyCap: Number(process.env.ACTIVITY_INDEX_MONTHLY_CAP || env.ACTIVITY_INDEX_MONTHLY_CAP || DEFAULT_MONTHLY_CAP),
  });
}
...
function describeRejection(c) {
  ...
  if (k.declared_by_homepage && k.name_matches_site === false) why.push(`its name does not appear in the domain or page title (a footer link to whoever built the site, most likely)`);
