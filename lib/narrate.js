...
export async function narrateFindings(findingsResult, { apiKey = process.env.ANTHROPIC_API_KEY, domain = null, timeoutMs = 20000, fetchImpl = fetch } = {}) {
  ...
  const prompt = `You are polishing findings for a website-readiness report about ${domain || 'a company'}. For each finding below, rewrite "statement" and "fix" to be tighter and more direct. HARD RULES: keep every number, count, percentage and URL exactly as given; add no facts, no numbers, no URLs, no speculation; keep the factual, non-mocking tone; one short paragraph each. Return ONLY a JSON array of {"i": <index>, "statement": "...", "fix": "..."}.\n\n${JSON.stringify(items, null, 1)}`;
