...
function cultureCareersFindings(ev, coverage) {
  ...
  if (cc && coverageOk) {
    ...
    if (culture === 0) {
      out.push(mk({
        // impact 4 -> 3 (2026-09-16): culture pages are now half of the category,
        // the other half being what employees actually do, reported separately.
        id: 'no_culture_pages', severity: 'opportunity', impact: 3, effort: 4,
        title: 'No culture or life-at pages anywhere in your sitemap',
        statement: `Of ${cc.canonical_urls?.toLocaleString('en-US')} canonical URLs, zero classify as culture content — no life-at pages, employee stories, or team spotlights. This is the most common gap this tool finds, and the most directly advocacy-shaped one: it is the content employees share proudest and first. Your people's own posting is scored separately, below.`,
...
// From the licensed index (lib/activity-index.js). Two findings, worded so a
// zero never reads as an accusation: the index is a lower bound with uneven
// coverage, and during testing it missed a company that posts weekly.
// CORPUS_MEDIAN_RATE is measured (123 gradeable domains, 2026-09-16), not chosen.
const CORPUS_MEDIAN_RATE = 6.1;
const QUIET_RATE = 3; // roughly the bottom quartile of the same measurement (p25 = 3.9)
function activityFindings(ev) {
  const a = ev.activity_index;
  const c = a?.counts;
  if (!a || a.status !== 'resolved' || !c || c.employees_indexed === null || c.employees_posted_in_window === null) return [];
  const out = [];
  const idx = c.employees_indexed;
  const rate = c.active_poster_rate_pct;
  if (idx >= 25 && rate !== null && rate < QUIET_RATE) {
    const dm = c.decision_makers_posted_in_window;
    out.push(mk({
      id: 'employees_quiet', severity: 'opportunity', impact: 4, effort: 3,
      title: 'Your people are barely posting',
      statement: `In the licensed index, ${c.employees_posted_in_window.toLocaleString('en-US')} of the ${idx.toLocaleString('en-US')} employees it has records for posted anything in the last ${c.window_days} days (${rate}%). That is a share of the index's coverage, not of your headcount. Across the 124 graded companies in this corpus with comparable counts the median is ${CORPUS_MEDIAN_RATE}%. This is a third-party count and a lower bound — but the gap to the median is not a rounding error.`,
      evidence: { source: 'licensed third-party index', employees_indexed: idx, posted_in_window: c.employees_posted_in_window, rate_pct: rate, corpus_median_pct: CORPUS_MEDIAN_RATE, window_days: c.window_days },
      fix: dm ? `This is the gap an advocacy program closes directly. Start with the ${dm} decision-maker(s) who already post: give them something from your site worth sharing this week.` : 'This is the gap an advocacy program closes directly. Start with the few people who already post: give them something from your site worth sharing this week.',
    }));
  }
  if (c.company_posts_in_window === 0) {
    out.push(mk({
      // 'info', not 'opportunity': index coverage of company pages is thin enough
      // that this fired on a company known to post weekly. Reported under
      // "For the record", never ranked as an action (QA 2026-09-17, P2-4).
      id: 'company_page_quiet', severity: 'info', impact: 2, effort: 2,
      title: 'No company-page posts in the last 90 days — in the index',
      statement: `${c.company_posts_all_time === null || c.company_posts_all_time === undefined ? 'The licensed index answered no total for your company page, and holds no posts from it' : `The licensed index holds ${c.company_posts_all_time} post(s) from your company page in total and none`} in the last ${c.window_days} days. That may be index coverage rather than silence — during testing the index missed a company that posts weekly — so treat this as a prompt to check, not a verdict.`,
      evidence: { source: 'licensed third-party index', company_posts_all_time: c.company_posts_all_time, company_posts_in_window: 0, window_days: c.window_days },
      fix: 'If the page really has gone quiet, employees have nothing of yours to amplify. A weekly cadence is the floor most graded companies clear.',
    }));
  }
  return out;
}
