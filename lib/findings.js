...
function activityFindings(ev) {
  ...
  if (idx >= 25 && rate !== null && rate < QUIET_RATE) {
    ...
    out.push(mk({
      ...
      statement: `In the licensed index, ${c.employees_posted_in_window} of ${idx.toLocaleString('en-US')} current employees posted anything in the last ${c.window_days} days (${rate}%). Across the 126 graded companies in this corpus the median is ${CORPUS_MEDIAN_RATE}%. This is a third-party count and a lower bound — but the gap to the median is not a rounding error.`,
  ...
  if (c.company_posts_in_window === 0) {
    out.push(mk({
      ...
      statement: `The licensed index holds ${c.company_posts_all_time ?? 0} post(s) from your company page in total and none in the last ${c.window_days} days. That may be index coverage rather than silence — during testing the index missed a company that posts weekly — so treat this as a prompt to check, not a verdict.`,
