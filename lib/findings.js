...
// observation: what we saw, where we saw it (URLs), what to change, and how
...
function fastFindings(ev) {
  ...
  if (robots && !robots.present) {
    out.push(mk({
      ...
      statement: `The robots.txt endpoint did not return a usable file${robots.reason ? ` (${robots.reason})` : ''}. Crawler and AI-agent policy for this domain is unknown — unknown is not the same as open, and we have not scored it as open.`,
  ...
  if (ev.llms_txt?.checked) {
    out.push(mk({
      ...
      statement: ev.llms_txt.present
        ? 'An llms.txt file is served. Reported for completeness: Google has said it does not support llms.txt, no major AI vendor has confirmed consuming it, and large-scale studies found no correlation with AI citations. We record it and weight it at zero.'
        : 'No llms.txt file is served. This costs nothing today — the evidence that llms.txt affects anything is weak — so we do not count it against you.',
  ...
  if (hp) {
    ...
    if (!ev.blocked_at_root && ev.meta) {
      out.push(mk({
        ...
        title: 'We could not retrieve your homepage',
        statement: 'No candidate origin returned a successful response to a polite, honestly-identified request — directly or through a rendering proxy. Everything a homepage would show us (share tags, structured data, careers links) is unobserved, not judged.',
...
function contentFindings(ev, coverage) {
  ...
  if (ev.sitemaps && ev.sitemaps.urls_collected === 0) {
    out.push(mk({
      ...
      statement: `We looked for sitemaps at ${(ev.sitemaps.seeds_tried || []).length} location(s) (declared in robots.txt or at the conventional paths) and collected zero URLs. Your content inventory is invisible to us — and to anything else that discovers content this way. This means "not found where sitemaps usually live", not "no content".`,
  ...
  // cited so the company can see we know the content exists.
  if (coverage?.insufficient) {
    ...
    out.push(mk({
      ...
      title: 'Our reader could not classify most of your site',
      statement: `Of the ${coverage.total_urls.toLocaleString('en-US')} URLs in your sitemap, ${coverage.unclassified_urls.toLocaleString('en-US')} (${coverage.unclassified_share_pct}%) could not be sorted into content sections by our path-based classifier${top.length ? ` — the unread majority sits under paths like ${top.join(', ')}` : ''}. That is a limit of our reading, not evidence you lack content, so no letter grade is issued and the volume-based scores below are floors, not measurements.`,
  ...
  if (coverageOk) {
    ...
    const sectionsPresent = Object.entries(cc.sections || {}).filter(([k, v]) => v > 0 && !['unclassified', 'legal', 'product', 'home', 'careers', 'partner_integration'].includes(k));
    ...
    if (!rec.recency_measurable) {
      out.push(mk({
        ...
        statement: rec.lastmod_looks_machine_generated
          ? `Your sitemap publishes lastmod dates, but they look machine-generated rather than edit history (${(rec.credibility_reasons || []).join('; ') || 'implausible date distribution'}), so we cannot read them as freshness and have not guessed.`
          : 'Your sitemap carries no lastmod dates, so we cannot see how fresh your content is and have not guessed.',
...
function pageSampleFindings(ev, coverage) {
  ...
  if (attempted > 0 && n === 0) {
    out.push(mk({
      ...
      statement: `We sampled ${attempted} content page(s) from your sitemap and none returned readable HTML. Everything a content page would show — share tags, authors, schema — is unobserved for this site.`,
  ...
  const caveat = coverage?.insufficient ? ' (sampled from the slice of the site we could read)' : '';
  ...
  for (const [key, label, impact, effort, why, fix] of pctChecks) {
    ...
    if (agg.pct === 0) {
      out.push(mk({
        ...
        statement: `None of the ${n} content pages we sampled${caveat} carry ${label}. ${why}`,
  ...
  if (cc && coverageOk) {
    ...
    if (culture === 0) {
      out.push(mk({
        ...
        statement: `Of ${cc.canonical_urls?.toLocaleString('en-US')} canonical URLs, zero classify as culture content — no life-at pages, employee stories, or team spotlights. This is the most common gap we see, and the most directly advocacy-shaped one: it is the content employees share proudest and first.`,
  ...
  if (checked.length) {
    ...
    if (!ok.length) {
      out.push(mk({
        ...
        title: 'We could not retrieve a careers page',
        statement: `We tried ${checked.length} careers location(s) (${checked.map((c) => c.url).filter(Boolean).slice(0, 3).join(', ')}) and none returned a readable page. Candidates researching you through tools and AI assistants hit the same wall.`,
