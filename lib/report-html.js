...
export const BRAND = `<p class="kicker brand">${MARK}<span class="wm"><span class="a">justme</span><span class="b">social</span></span><span class="brand-sep" aria-hidden="true"></span><span class="brand-prod">Advocacy Grade</span></p>`;
...
:root{
  ...
  --ghost:#c3c9e8;   /* the two bubbles the mark gives up */
...
.kicker.brand{display:flex;align-items:center;gap:3px;flex-wrap:wrap;
  font-size:19px;line-height:1;letter-spacing:-.005em;text-transform:none;color:var(--muted)}
.mark{display:block;flex:0 0 auto;margin:-1px 0}
...
.brand-sep{width:1px;height:15px;background:var(--line-strong);margin:0 11px}
.brand-prod{font-size:11px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--muted)}
...
function subScoreGrid(scoring) {
  ...
  const cards = Object.keys(CAT_LABEL).map((key) => {
    ...
    return `<div class="sub-card b-${band}">
      ${scored
        ? `<div class="num score-num">${c.score}<span class="den">/100</span></div>`
        : '<div class="num">Not scored</div>'}
