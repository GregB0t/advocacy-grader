...
import { indexFromEnv, observeActivity } from './activity-index.js';
...
export function normalizeDomain(input) {
  ...
  // A trailing dot is a valid FQDN spelling but a distinct cache key, report URL
  // and <h1> if left in place (QA 2026-09-17 found "hubspot.com." grading twice).
  const host = u.hostname.toLowerCase().replace(/\.+$/, '');
  // Bare integers ("12345") parse as an IPv4 literal ("0.0.48.57") and were
  // rejected with an IP-specific message; treat them as unparseable instead.
  if (/^\d+$/.test(String(input || '').trim())) return null;
  // Empty labels ("hubspot..com") and the RFC 2606 reserved TLDs can never
  // resolve — fail at parse rather than spend a crawl and a rate-limit slot.
  if (host.split('.').some((l) => l === '')) return null;
  if (/\.(test|example|invalid|localhost)$/.test(host)) return null;
  const origin = u.origin.replace(u.hostname, host);
  return { input, scheme: u.protocol.replace(':', ''), host, port: u.port || null, origin, explicitScheme };
}
...
export async function run(opts) {
  ...
  // 5a. Employee posting activity from the licensed index (lib/activity-index.js).
  // Started here and awaited at the end so its ~2-4s overlaps the sitemap and
  // page work. The homepage HTML is the resolver's fallback (the LinkedIn
  // /company/ link the site itself declares). Never throws; failure lands in
  // not_observed and the rubric scores the category on website evidence alone.
  const activityPromise = observeActivity(opts.activityIndex ?? indexFromEnv(envFile), {
    domain: norm.host,
    finalHost: (() => { try { return new URL(origin).hostname; } catch { return null; } })(),
    html: resolved.homepage?.ok ? resolved.homepage.body : null,
  });
  ...
  return envelope({
    ...
    languageResolution, shareability, leadSignals, bee, robotsReg, activity,
  });
}
...
function envelope(ctx) {
  ...
  // Employee posting activity: attached in run() before envelope() on the main
  // path. The two early-return envelopes (site unreachable, blocked at root)
  // carry an explicit not-looked-up block so the key is always present and
  // never silently absent.
  if (activity) {
    out.activity_index = activity.block;
    notObserved.push(...activity.not_observed);
  } else {
    out.activity_index = { source: 'licensed third-party index', status: 'not_looked_up', reason: blockedAtRoot ? 'the site blocked this crawler at the root, so no report was assembled beyond robots.txt' : 'the site could not be reached' };
    notObserved.push('Employee posting activity: not looked up, because the report stopped before a company could be identified.');
  }
